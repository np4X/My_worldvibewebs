...
% circuit diagram, graph, or a scan from the source notes.
...
\blankfig[4cm]{3cm}{Black-box step-down converter: $V_{in}=50\,\text{V}$ in, unknown network, $\langle V \rangle = 20\,\text{V}$ across the load.}
\end{frame}
...
\blankfig[6cm]{3cm}{Ideal switch model: pole connects to $V_{in}$ for a
...
\blankfig[8cm]{3.5cm}{Buck converter: switch $D$ followed by an $LC$
...
\blankfig[8cm]{3.5cm}{Steady-state inductor current: triangular ripple of
...
\blankfig[8cm]{3cm}{Buck circuit during the ON interval: $V_{in}=50\,\text{V}$, $D=0.5$, $L=10\,\text{mH}$, $T_s=1\,\text{ms}$, output branch at $V=0$.}
\end{frame}
...
\blankfig[8cm]{3.5cm}{Buck circuit during the OFF interval, same
...
\blankfig[8cm]{3.5cm}{Buck circuit with increased output current
...
\blankfig[9cm]{3.8cm}{Classic multi-period inductor current waveform
$i_L(t)$ ramping toward steady state (cf.\ Erickson, \emph{Fundamentals
...
\blankfig[9cm]{3.5cm}{Replacing the ideal switch with a MOSFET (A--B)
...
\blankfig[7cm]{3.5cm}{$I_{AB}$--$V_{AB}$ switch locus for the transistor
...
\blankfig[7cm]{3.5cm}{$I_{CB}$--$V_{CB}$ switch locus for the diode
position: blocks reverse voltage (negative $V_{CB}$), conducts forward
current.}
\end{frame}
...
\blankfig[8cm]{5.5cm}{Idealized switch characteristics for BJT/IGBT,
MOSFET, and diode (textbook reference figures).}
\end{frame}

\begin{frame}{Buck: Summary}
\blankfig[8cm]{3.5cm}{Final buck converter circuit: MOSFET (A--B),
